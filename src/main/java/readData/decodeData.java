...
public class decodeData {
	...
	/*
	 * public static void main(String[] args){ PrintMatrix();
	 * 
	 * 
	 * }
	 */
	...
	/*
	 * public static float CalAverageRate(int i){ ArrayList ratedMatrix =
	 * MakeRatedMatrix(); int total = 0; int count = 0; for(Object x:ratedMatrix){
	 * ArrayList y = (ArrayList) x; for (Object y1 : y) { ArrayList rate =
	 * (ArrayList) y1; int idFilm = (int) rate.get(0); int point = (int)
	 * rate.get(1); if(idFilm == i){ total += point; count++; break; }else continue;
	 * } } float AverageRate = (float)(total/count); return AverageRate; }
	 */
	...
	public static ArrayList BothRate(int i, int j) {
		ArrayList ratedMatrix = MakeRatedMatrix();
		ArrayList bothRateMatrix = new ArrayList();// arraylist lưu trữ những người sử dụng cùng đánh giá i và j, lưu
													// trữ tất cả nhg film người đó đã đánh giá bao gồm cả i, j
		for (Object x : ratedMatrix) {
			int count = 0;
			ArrayList y = (ArrayList) x;
			for (Object y1 : y) {
	...
	/*
	 * public static void main(String[] args){ ArrayList a = BothRate(1,2);
	 * 
...
/*
 * public static void main(String[] args){ float ts = CalFirst(1,1); float ms =
 * CalSecond(1,1); float a = ts/ms; System.out.println(a);
 * System.out.println(ts); System.out.println(ms); } }
 */

// ma trận tương đồng giữa các bộ film
/*
 * public static ArrayList DissMatrix(){ ArrayList dissmatrix = new ArrayList();
 * for(int i=1;i < 1682;i++){ ArrayList feature = new ArrayList(); for(int k=1;k
 * < 1682;k++){ float diss = CalDis(i,k); feature.add(diss); }
...
/*
 * public static float calDis() public static void main(String[] args){
 * ArrayList test = MakeRatedMatrix(); PrintMatrix(); }
 * 
 * 
 * 
 * 
 ...
 * }
 * 
 * /*public static void main(String[] args){ ArrayList NewData = DecodeData();
 * for(int k=0;k < NewData.size();k++){ System.out.println(NewData.get(k)); }
 */

/*
 * String item2 = line.substring(4,8); String item3 = line.substring(8,9);
 * String item4 = line.substring(9,18); newLine.add(item1); newLine.add(item2);
 * newLine.add(item3); newLine.add(item4); item.add(newLine);
